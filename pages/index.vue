    ...
    if (equal.value) {
      ...
      if (tab.value == "file") {
        ...
        reader.onload = async (e) => {
          ...
          await token2.increaseAllowance(
            transfer.target,
            ethers.parseEther(sum.toString())
          );
          await transfer.multiTransfersTokensEqualAmount(
            tokenAddress.value,
            addresses,
            ethers.parseEther(amounts)
          );
        };
      ...
      if (tab.value == "file") {
        ...
        reader.onload = async (e) => {
          ...
          await transfer.multiTransfersTokens(
            tokenAddress.value,
            addresses,
            amounts
          );
  ...
  if (equal.value) {
    ...
    if (tab.value == "file") {
      ...
      reader.onload = async (e) => {
        json = JSON.parse(e.target.result);
        addresses = json.addresses;
        await transfer.multiTransfersEthEqualAmount(
          addresses,
          ethers.parseEther(amounts.toString()),
          {
            value: ethers.parseEther(
              (parseFloat(amounts) * addresses.length).toString()
            ),
          }
        );
      };
            ...
            >
              ...
              <q-card-section>
                <q-tabs v-model="tab" active-color="primary">
                  <q-tab name="input"> Manual</q-tab>
                  <q-tab name="file"> From file</q-tab>
                </q-tabs>
                <q-toggle
                  keep-color
                  color="primary"
                ...
                <q-input
                  ...
                  dense
                />
                <q-tab-panels v-model="tab" style="background-color: transparent">
                  <q-tab-panel name="input">
                    <q-input
                      ...
                      type="textarea"
                      outlined
                      :placeholder="`Insert allocation: separate (;). By format: address${
                        equal ? '' : ';amount'
                      }
Example:
